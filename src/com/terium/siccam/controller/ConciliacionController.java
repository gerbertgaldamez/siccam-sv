...
public class ConciliacionController extends MenuController {
	...
	private static Logger log = Logger.getLogger(ConciliacionController.class);
	...
	public void cargarComboAgencia() {
		try {
			...
			log.debug("cargarComboAgencia()  " + " - Llena combo tipo");
		} catch (Exception e) {
	...
	public void cargarComboEstado() {
		try {
			...
			log.debug("cargarComboEstado()  " + " - Llena combo estado");
		} catch (Exception e) {
			Messagebox.show("Se ha producido un error.", "ATENCION", Messagebox.OK, Messagebox.ERROR);
			log.error("cargarComboEstado() -  Error ", e);
		}
	}
	...
	public void listarTodasConciliaciones(String agencia, String tipo, String estado, String codigoColector) {
		try {
			...
			log.debug("listarTodasConciliaciones() - " + "Fecha inicial para consulta " + dia);
			log.debug("listarTodasConciliaciones() - " + "Fecha final para consulta " + dia2);
			...
			log.debug("listarTodasConciliaciones() - obtiene conciliaciones");
			...
			log.error("listarTodasConciliaciones() -  Error", e);
		}
	}
			...
			try {
				if (lst.size() > 0) {
					...
					while (ilst.hasNext()) {
						...
						cell = new Listcell();
						cell.setStyle("text-align: right");
						// BigDecimal noConciliadoBanco = new
						// BigDecimal(convertirADecimal(adr.getPendienteBanco()));
						// BigDecimal noConciliadoTelca = new
						// BigDecimal(convertirADecimal(adr.getPendienteTelefonica()));
						...
						// esta validacion esta para sv, ni y pa, para GT y CR no esta se realizan
						// pruebas para ver si se debe tomar en cuenta.
						...
						if ((adr.getManualTelefonica().compareTo(BigDecimal.ZERO) > 0)
						...
						if ((adr.getPagosTelefonica().compareTo(BigDecimal.ZERO) == 0)
								&& (adr.getConfrontaBanco().compareTo(BigDecimal.ZERO) != 0)) {
						...
						fila.addEventListener(Events.ON_DOUBLE_CLICK, new EventListener() {
	...
	public void listarConciliacionesAux(List<CBResumenDiarioConciliacionModel> lista) {
		...
		try {
			...
			try {
				...
				log.error("listarConciliacionesAux() - error :  ", e);
			}
		} catch (Exception e) {
			log.error("listarConciliacionesAux() - error list :  ", e);
		}
	...
	public void abrirConciliacionDetalle() {
		...
		try {
			...
			if (lbxConciliacion.getSelectedItem() != null) {
				...
				num = con.getDia().getDay() + "-" + con.getDia().getMonth() + "- Agen: " + con.getIdAgencia() + " - "
						+ con.getTipo();
				super.llamarConciliacionDetalle(num);
			}

		} catch (Exception e) {
			log.error("abrirConciliacionDetalle() - error :  ", e);
		}
	}
	...
	public void onClick$btnBuscar() {
		log.debug("onClick$btnBuscar() - " + "Combo agencia: " + cmbAgencia.getText() + " estado: "
				+ cmbEstado.getText() + " cmbTipo: " + cmbTipo.getText());
		...
		/**
		 * Agregado por Nicolas Bermudez - Qitcorp 19/09/2017 Parseo de fechas
		 */
		...
		try {
			...
			log.error("onClick$btnBuscar() - Error : ", e);
		}
		/**
		...
		/**
		 * Agrega CarlosGodinez -> 07/08/2018 Se agregago codigo de colector como
		 * parametro
		 * 
		 */
		...
		log.debug(
				"onClick$btnBuscar() - " + "FILTROS: agencia = " + agencia + " tipo = " + tipo + " estado = " + estado);
	...
	public void onClick$btnGeneraReporte() {
		log.debug("onClick$btnGeneraReporte() - inicia generar reporte");
		if (dtbDia.getValue() == null) {
			Messagebox.show("Debe de seleccionar un rango de fecha para el reporte.", "ATENCIÓN", Messagebox.OK,
					Messagebox.EXCLAMATION);

		} else if (dtbDia.getValue().after(dtbDia2.getValue())) {
			Messagebox.show("La fecha desde debe ser menor a la fecha hasta.", "ATENCIÓN", Messagebox.OK,
					Messagebox.EXCLAMATION);

		} else {
			log.debug("onClick$btnGeneraReporte() - " + "fecha: " + fechaFormato.format(dtbDia.getValue()));
			generarTxtConciliaciones();
			log.debug("onClick$btnGeneraReporte() -  Finaliza generar reporte");
		}
	...
	private String QRY = "SELECT dia, " + "nombre, " + "tipo, " + "trans_telefonica transTelefonica, "
			+ "trans_banco transBanco, " + "conciliadas, " + "dif_trans difTransaccion, "
			+ "pagos_telefonica pagosTelefonica, " + "confronta_banco confrontaBanco, " + "automatica, "
	 ...
	 * 
	 * List<CBResumenDiarioConciliacionModel> listado = new
	 * ArrayList<CBResumenDiarioConciliacionModel>();
	 * 
	 * String fechaDesde = fechaFormato.format(dtbDia.getValue()); String fechaHasta
	 * = fechaFormato.format(dtbDia2.getValue());
	 * 
	 * String Where = "";
	 * 
	 ...
	 * 
	 * if (!"Todas".equals(cmbAgencia.getText().trim())) { CBCatalogoAgenciaModel
	 * agen = cmbAgencia.getSelectedItem() .getValue();
	 * 
	 * String idAgencia = agen.getcBCatalogoAgenciaId();
	 * Logger.getLogger(ConciliacionController.class.getName()).log(Level.INFO,
	 * "agencia: " + idAgencia); Where += " and cbcatalogoagenciaid = '" + idAgencia
	 * + "' ";
	 * 
	 * }
	 * 
	 * if (!"Todos".equals(cmbTipo.getText().trim())) { String tipo =
	 * cmbTipo.getSelectedItem().getLabel();
	 * Logger.getLogger(ConciliacionController.class.getName()).log(Level.INFO,
	 * "tipo: " + tipo); Where += " and tipo = '" + tipo + "' "; }
	 * 
	 * if (!"Todos".equals(cmbEstado.getText().trim())) { String op =
	 * cmbEstado.getText();
	 * 
	 * if ("Conciliado Automaticamente".equals(op)) { Where +=
	 * " and pagos_telefonica = confronta_banco "; } if
	 * ("Pendiente Conciliar".equals(op)) { Where +=
	 * " AND pagos_telefonica >0 and confronta_banco > 0  " +
	 * " and ( confronta_banco-automatica-manual_b > 0 OR pagos_telefonica-automatica-manual_t > 0 )"
	 * ; } if ("Conciliado Manualmente".equals(op)) { Where +=
	 * " and (confronta_banco-automatica-manual_b = 0 and manual_t > 0) or (pagos_telefonica-automatica-manual_t=0 and manual_b > 0) "
	 * ; } if ("Pendiente Pagos".equals(op)) { Where +=
	 * " and pagos_telefonica = 0 "; } if ("Pendiente Confronta".equals(op)) { Where
	 * += " and confronta_banco = 0 "; }
	 * 
	 * }
	 * 
	 * 
	 * Logger.getLogger(ConciliacionController.class.getName()).log(Level.INFO,
	 * "Query reporte conciliaciones: " + QRY + Where); try { Connection con =
	 * obtenerDtsPromo().getConnection(); try { QueryRunner qr = new QueryRunner();
	 * BeanListHandler<CBResumenDiarioConciliacionModel> bhl = new
	 * BeanListHandler<CBResumenDiarioConciliacionModel>(
	 * CBResumenDiarioConciliacionModel.class); listado = qr.query(con, QRY + Where,
	 * bhl, new Object[] {}); } finally { if(con != null) con.close(); } } catch
	 * (Exception e) {
	 * Logger.getLogger(ConciliacionController.class.getName()).log(Level.SEVERE,
	 * null, e); }
	 * 
	 * return listado; }
	 */
	...
	public void generarTxtConciliaciones() {
		...
		try {
			if (lst != null)
				log.debug("generarTxtConciliaciones()" + " - lst size " + lst.size());

			if (lst != null && lst.size() > 0) {
				log.debug("generarTxtConciliaciones()" + " -  generando Reporte");
				...
				log.debug("generarTxtConciliaciones()" + " - Descargando el archivo");
				...
				log.debug("generarTxtConciliaciones()" + " - No hay datos generados en la busqueda");
				Messagebox.show("Debe consultar datos antes de generar reportes.", "ATENCIÓN", Messagebox.OK,
						Messagebox.EXCLAMATION);
			}
		} catch (Exception e) {
			Messagebox.show("Se ha producido un error.", "ATENCION", Messagebox.OK, Messagebox.ERROR);
			log.error("generarTxtConciliaciones() - Error : ", e);
		} finally {
			if (bw != null)
				try {
					bw.close();
				} catch (IOException e) {
					log.error("generarTxtConciliaciones() - Error close bw : ", e);
				}
		}
	 ...
	 * Logger.getLogger(ConciliacionController.class.getName()).log(Level.INFO,
	 * "comienza a generar el reporte..."); if (dtbDia.getValue() == null) {
	 * Messagebox.show( "Debe de seleccionar un rango de fecha para el reporte.",
	 * "ATENCION", Messagebox.OK, Messagebox.EXCLAMATION); } else if
	 * (dtbDia.getValue().after(dtbDia2.getValue())) {
	 * Messagebox.show("La fecha desde debe ser menor a la fecha hasta.",
	 * "ATENCION", Messagebox.OK, Messagebox.EXCLAMATION); } else {
	 * Logger.getLogger(ConciliacionController.class.getName()).log(Level.INFO,
	...
	private static String QRYDOS = "SELECT DISTINCT dia||'|'|| " + "b.nombre||'|'|| "
			+ "DECODE (a.tipo, '1', 'PRE-PAGO', '2', 'POST-PAGO')||'|'|| " + "(SELECT NVL(COUNT(d.imp_pago),0) "
			+ "FROM cb_pagos d " + "WHERE d.tipo   = a.tipo " + "AND d.cod_caja = m.cod_agencia "
			+ "AND a.dia      = d.fecha " + ")||'|'|| " + "(SELECT NVL( SUM (d.imp_pago),0) " + "FROM cb_pagos d "
			+ "WHERE d.tipo   = a.tipo " + "AND d.cod_caja = m.cod_agencia " + "AND a.dia      = d.fecha " + ")  "
			+ "FROM cb_conciliacion a, " + "  cb_catalogo_agencia b, " + "  cb_banco_agencia_confronta m "
			+ "WHERE a.cbcatalogoagenciaid = b.cbcatalogoagenciaid "
			+ "AND a.cbcatalogoagenciaid   = m.cbcatalogoagenciaid " + "AND a.tipo                  = m.tipo ";

	/*
	 * public void generarTxtConciliacionesDos() {
	 * 
	 * Date fecha = new Date(); SimpleDateFormat sdf = new
	 * SimpleDateFormat("yyyyMMdd_HH_mm_ss"); String fechaDesde =
	 * fechaFormato.format(dtbDia.getValue()); String fechaHasta =
	 * fechaFormato.format(dtbDia2.getValue()); String nombreArchivo =
	 * "reporte_conciliaciones_" + sdf.format(fecha) + ".txt"; QRYDOS +=
	 * " and a.dia >= to_date('" + fechaDesde + "', 'dd/MM/yyyy')" +
	 * " and a.dia <= to_date('" + fechaHasta + "', 'dd/MM/yyyy') ";
	 * 
	 * String usuario = obtenerUsuario().getUsuario(); String correo =
	 * obtenerUsuario().getEmail();
	 * 
	 * Logger.getLogger(ConciliacionController.class.getName()).log(Level.INFO,
	 * "query: " + QRYDOS);
	 * Logger.getLogger(ConciliacionController.class.getName()).log(Level.INFO,
	 ...
	 * 
	 * CallableStatement callableStatement = null; try { Connection con =
	 * obtenerDtsPromo().getConnection(); try { callableStatement = con
	 * .prepareCall("{call PR_GENERA_REPORTE  (?, ?, ?, ?, ?)}");
	 * callableStatement.setString(1, QRY); callableStatement.setString(2,
	 * CAMPOS_REPORTE_CONCILIACIONES); callableStatement.setString(3, usuario);
	 * callableStatement.setString(4, nombreArchivo); callableStatement.setString(5,
	 * correo); callableStatement.executeUpdate();
	 * 
	 * // ejecuta shell para el envio de reporte ejecutaSh();
	 * 
	 * Messagebox
	 * .show("Su repote le estara llegando en unos momentos a su correo.",
	 * "ATENCION", Messagebox.OK, Messagebox.EXCLAMATION); } finally {
	 * if(callableStatement != null) callableStatement.close(); if(con != null)
	...
	public void ejecutaSh() throws JSchException {
		...
		List<ListaCombo> configuraciones = new CBConciliacionDAO().listadoParametros();
		String ipFTP = "";
		String usuarioFTP = "";
		String passFTP = "";
		...
		try {
			...
			log.debug("ejecutaSh()" + " - Conectado: " + login);
			if (login) {
				ejecutaComandoSSH(usuarioFTP, passFTP, ipFTP, "cd " + directorioFTP + "; " + "./genera_reporte.sh");
			} else {
				log.debug("ejecutaSh()" + " - No se puede conectar FTP");
			}
			client.logout();
			client.disconnect();

		} catch (SocketException e) {
			log.error("ejecutaSh() - Error Socket Exception :", e);
		} catch (IOException e) {
			log.error("ejecutaSh() - Error IoExcp. :", e);
		}

	}

	public void ejecutaComandoSSH(String sshuser, String sshpass, String sshhost, String command)
			throws JSchException, IOException {
		List<String> results = new ArrayList<String>();
		log.debug("ejecutaComandoSSH()" + "- ejecutaComandoSSH start " + Calendar.getInstance().getTime());
		log.debug("ejecutaComandoSSH()" + " - " + "\tcommand -> " + command);
		...
		JSch jsch = new JSch();
		Session session;
		log.debug("ejecutaComandoSSH()" + " - sshuser : " + sshuser);
		log.debug("ejecutaComandoSSH()" + " - sshhost : " + sshhost);
		log.debug("ejecutaComandoSSH()" + " - sshpass : " + sshpass);
		...
		log.debug("ejecutaComandoSSH()" + " - conectado!" + session.isConnected());
		...
		log.debug("ejecutaComandoSSH()" + " - channel conectado! " + channel.isConnected());
		...
		while (true) {
			while (in.available() > 0) {
				log.debug("ejecutaComandoSSH()" + " - Ingreso OK");
				int i = in.read(tmp, 0, 1024);
				log.debug("ejecutaComandoSSH()" + " - valor de i: " + i);
				if (i < 0) {
					log.debug("ejecutaComandoSSH()" + " - i es menor a 0 ");
					break;
				}
				String buffer = new String(tmp, 0, i);
				results.add(buffer);
				log.debug("ejecutaComandoSSH()" + " - resultados: " + results.get(0));
				log.debug("ejecutaComandoSSH()" + " - buffer contains: " + buffer.intern());
		...
		channel.disconnect();
		session.disconnect();
		log.debug("ejecutaComandoSSH()" + " - " + "\trunCommandViaSSH end " + Calendar.getInstance().getTime());
