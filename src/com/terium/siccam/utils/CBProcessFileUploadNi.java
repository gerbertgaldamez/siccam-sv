...
import com.terium.siccam.service.ProcessFileTxtServImplNI;

public class CBProcessFileUploadNi {
	...
	public void mapeoCargaConfrontasNI() {
		...
		if ("txt".equals(media.getFormat()) || format.toUpperCase().equals("log".toUpperCase())
			...
			Logger.getLogger(CBProcessFileUploadNi.class.getName()).log(Level.INFO, null,
			...
			Logger.getLogger(CBProcessFileUploadNi.class.getName()).log(Level.INFO,
			...
			Logger.getLogger(CBProcessFileUploadNi.class.getName()).log(Level.INFO, "id idBAC: " + idBAC);
			BigDecimal comision = bancoDAO.recuperaComision(idBAC);
			misession.setAttribute("comisionConfronta", comision);
			misession.setAttribute("cbBancoAgenciaConfrontaId", idBAC);
			...
			try {
				...
				Logger.getLogger(CBProcessFileUploadNi.class.getName()).log(Level.SEVERE, null, e);
			} catch (IOException e) {
				Logger.getLogger(CBProcessFileUploadNi.class.getName()).log(Level.SEVERE, null, e);
			...
			try {
				...
				Logger.getLogger(CBProcessFileUploadNi.class.getName()).log(Level.INFO, null, e);
				is = media.getStreamData();
			} catch (Exception e) {
				System.out.println("Ha ocurrido un error: " + e.getMessage());
				Messagebox.show("Ha ocurrido un error al intentar cargar el archivo de confronta", "ATENCION",
	...
	private void leerArchivoTxt(int idBanco, int idAgencia, int idConfronta, String user, String nombreArchivo,
		...
		try {
			ProcessFileTxtService fileTxtService = new ProcessFileTxtServImplNI();
	...
	public void grabarData() throws ParseException, SQLException, NamingException {
		...
		if (cbdb.consultaExistenciaArchivo(nombreArchivo, listDataBanco.get(0).getIdCargaMaestro())) {
			...
			Logger.getLogger(CBProcessFileUploadController.class.getName()).log(Level.INFO, null,
