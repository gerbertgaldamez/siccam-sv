...
public class CBProcessFileUploadCR {
	...
	public void mapeoCargaConfrontasCR() {
		...
		if ("txt".equals(media.getFormat()) || format.toUpperCase().equals("log".toUpperCase())
			...
			Logger.getLogger(CBProcessFileUploadCR.class.getName()).log(Level.INFO, null,
			...
			Logger.getLogger(CBProcessFileUploadCR.class.getName()).log(Level.INFO,
			...
			Logger.getLogger(CBProcessFileUploadCR.class.getName()).log(Level.INFO, "id idBAC: " + idBAC);
			BigDecimal comision = bancoDAO.recuperaComision(idBAC);
			misession.setAttribute("comisionConfronta", comision);
			misession.setAttribute("cbBancoAgenciaConfrontaId", idBAC);
			...
			SimpleDateFormat df = new SimpleDateFormat(formatoFechaConfronta, Locale.US);
			...
			try {
				...
				Logger.getLogger(CBProcessFileUploadCR.class.getName()).log(Level.SEVERE, null, e);
			} catch (IOException e) {
				Logger.getLogger(CBProcessFileUploadCR.class.getName()).log(Level.SEVERE, null, e);
	...
	private void leerArchivoTxt(int idBanco, int idAgencia, int idConfronta, String user, String nombreArchivo,
		...
		try {
			ProcessFileTxtService fileTxtService = new ProcessFileTxtServImplCR();
	...
	public void grabarData() throws ParseException, SQLException, NamingException {
		...
		System.out.println("id archivo: " + listDataBanco.get(0).getIdCargaMaestro());
...
*/
			...
			Logger.getLogger(CBProcessFileUploadCR.class.getName()).log(Level.INFO,
					"asi se manada la fecha formato:" + listDataBanco.get(0).getFormatofecha());
			...
			try {
				...
				/*
				 * con_thread = new ProcesaConciliacionThread(listDataBanco.get(0)
				 * .getIdCargaMaestro(), agencia_ID, conn,listDataBanco, listSinProcesarBanco,
				 * formatoFechaConfronta); con_thread.start();
				 */
