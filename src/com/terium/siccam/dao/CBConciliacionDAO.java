...
public class CBConciliacionDAO extends ControladorBase {
	...
	private static Logger log = Logger.getLogger(CBConciliacionDAO.class);
	...
	public int insertar(CBConciliacionModel registro) {
		...
		try {
			...
			queInsert = " Insert into  " + CBConciliacionModel.TABLE + " ("
					+ ObtieneCampos.obtieneSQL(CBConciliacionModel.class, null, true, null) + ") values " + " ("
					+ ObtieneCampos.obtieneInsert(CBConciliacionModel.class) + ")";

			QueryRunner qry = new QueryRunner();
			Object[] param = new Object[] { registro.getCBConciliacionId(), registro.getCodCliente(),
					registro.getTelefono(), registro.getFecha(), registro.getMonto(), registro.getEstado(),
					registro.getMes(), registro.getCreadoPor(), registro.getModificadoPor(),
			...
			try {
				...
				log.error("insertar() - Error conn close : ", e);
			}
		}
	...
	public int actualiza(CBConciliacionModel registro) {
		...
		try {
			...
			queInsert = " Update " + CBConciliacionModel.TABLE + " set " + CBConciliacionModel.FIELD_COD_CLIENTE
					+ " =?, " + CBConciliacionModel.FIELD_TELEFONO + " =?, " + CBConciliacionModel.FIELD_FECHA + " =?, "
					+ CBConciliacionModel.FIELD_MONTO + " =?, " + CBConciliacionModel.FIELD_ESTADO + " =?, "
					+ CBConciliacionModel.FIELD_MES + " =?, " + CBConciliacionModel.FIELD_CREADO_POR + " =?, "
					+ CBConciliacionModel.FIELD_MODIFICADO_POR + " =?, " + CBConciliacionModel.FIELD_FECHA_CREACION
					+ " =?, " + CBConciliacionModel.FIELD_FECHA_MODIFICACION + " =?, " + " where  "
					+ CBConciliacionModel.PKFIELD_CBCONCILIACIONID + " = ? ";

			QueryRunner qry = new QueryRunner();
			Object[] param = new Object[] { registro.getCodCliente(), registro.getTelefono(), registro.getFecha(),
					registro.getMonto(), registro.getEstado(), registro.getMes(), registro.getCreadoPor(),
			...
			try {
				...
				log.error("actualiza() - Error conn close : ", e);
			}
		}
	...
	public List<CBConciliacionModel> Listado(List<Filtro> filtro, List<Orden> orden) {
		...
		try {
			conn = obtenerDtsPromo().getConnection();
			query = "select " + ObtieneCampos.obtieneSQL(CBConciliacionModel.class, null, true, null) + " from "
			...
			if (filtroQuery.getParams() != null) {
				ret = (List<CBConciliacionModel>) qry.query(conn, query + sqlFiltros + filtroQuery.getSql(), blh,
			...
			try {
				...
				log.error("Listado() - Error conn close : ", e);
			}
		}
	...
	public List<CBResumenDiarioConciliacionModel> obtenerCBConciliaciones(CBResumenDiarioConciliacionModel objModel) {
		...
		ResultSet rst = null;
		log.debug("obtenerCBConciliaciones() " + " - Inicia obtener conciliaciones");

		try {
			conn = obtenerDtsPromo().getConnection();
			/**
			 * Modified by CarlosGodinez -> 10/08/2018 Se construye query de manera dinamica
			 * y mas limpia
			 */
			log.debug("obtenerCBConciliaciones() " + " - query consultsa : " + query);
			query = ConsultasSQ.RESUMEN_DIARIO_CONCILIACION_QRY;

			query += " and dia >= to_date('" + objModel.getFechaInicial() + "', 'dd/MM/yyyy')" + " and dia <= to_date('"
					+ objModel.getFechaFinal() + "', 'dd/MM/yyyy') ";
			...
			log.debug("obtenerCBConciliaciones() " + "Query resumen diario de conciliaciones: " + query);
			 ...
			 * CBResumenDiarioConciliacionModel.class); lst = qr.query(conn, query, bhl, new
			 * Object[] {});
			 * 
			...
			log.error("obtenerCBConciliaciones() - Error : ", e);
		} finally {
			try {
				if (conn != null)
					conn.close();
			} catch (SQLException e) {
				log.error("obtenerCBConciliaciones() - Error close conn : ", e);
			}
		}
		return lst;
	}
	...
	public List<ListaCombo> listadoParametros() {
		...
		try {
			...
			listado = qr.query(con, CONSULTA_PARAMETROS, bhl, new Object[] {});
		} catch (Exception e) {
			log.error("listadoParametros() - Error : ", e);
		} finally {
			try {
				if (con != null)
					con.close();
			} catch (SQLException e) {
				log.error("listadoParametros() - Error close conn : ", e);
			}
		}
		return listado;
	}
