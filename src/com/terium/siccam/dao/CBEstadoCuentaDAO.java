...
public class CBEstadoCuentaDAO extends ControladorBase {
	...
	/**
	 * Se eliminan todos los registros de una transaccion realizada para el
	 * mismo dia
	 */
	...
	/**
	 * @author Juankrlos - 11/01/2017 - Obtenemos las cuentas configurados para
	...
	//MODIFICA OVIDIO SANTOS
		//Este query es llamado desde la clase CBEstadoCuentaDAO.java por el metodo banco nacional
	...
	/**
	 * Inserta los registros para VISA
	 * Agregado por Juankrlos - 12/01/2017 -
	 * */	
	public boolean insertCuentasVisa(List<CBEstadoCuentasModel> list){
		...
		try {
			...
			if(contador > 0){
				Logger.getLogger(CBEstadoCuentaDAO.class.getName())
			    .log(Level.INFO,"Script ejecutado correctamente: "+contador);
	...
	/**
	 * Inserta los registros para VISA DETALLE
	 * Agregado por Juankrlos - 12/01/2017 -
	 * */	
	public boolean insertCuentasVisaDet(List<CBEstadoCuentasModel> list){
		...
		try {
			...
			if(contador > 0){
				Logger.getLogger(CBEstadoCuentaDAO.class.getName())
			    .log(Level.INFO,"Script ejecutado correctamente insert iva detalle: "+contador);
				result = true;
			}
		}catch(Exception e){
			Logger.getLogger(CBEstadoCuentaDAO.class.getName()).log(Level.SEVERE, null, e);
	...
	/**
	 * Inserta los registros para VISA DETALLE Agregado por Juankrlos -
	 * 12/01/2017 -
	 */
	...
	public boolean insertCuentasCredoEncabezadoCR(List<CBEstadoCuentasModel> list) {
		...
		try {
			...
			while (it.hasNext()) {
				...
				ps.setString(5, objModel.getConsumo());
				ps.setString(6, objModel.getComision());
				ps.setString(7, objModel.getIva());				
				ps.setString(8, objModel.getRetencion());
				ps.setString(9, objModel.getLiquido());
				ps.setString(10, objModel.getFechatransaccion());
				ps.setString(11, objModel.getCodigo_lote());
				...
				//nuevos
				ps.setString(16, objModel.getDocumento());
				ps.setString(17, objModel.getPropina());
				ps.setString(18, objModel.getCredito());
				ps.setString(19, objModel.getImpturismo());
				ps.setString(20, objModel.getIvacomision());
				ps.setString(21, objModel.getUsuario());
				ps.addBatch();
				contador++;
			}
...
public boolean insertCuentasCredoEncabezadoGT(List<CBEstadoCuentasModel> list){
		...
		try {
			...
			if(contador > 0){
				Logger.getLogger(CBEstadoCuentaDAO.class.getName())
				.log(Level.INFO,"Script ejecutado correctamente insert credomatic "+contador);
				result = true;
			}
		} catch (SQLException e) {
			Logger.getLogger(CBEstadoCuentaDAO.class.getName()).log(Level.SEVERE, null, e);
	...
	/**
	 * Inserta los registros para CREDOMATIC DETALLE
	 * Agregado por Juankrlos - 12/01/2017 -
	 * */	
	...
	public boolean insertCuentasCredoDetGT(List<CBEstadoCuentasModel> list){
		...
		try {
			...
			if(contador > 0){
				Logger.getLogger(CBEstadoCuentaDAO.class.getName())
				.log(Level.INFO,"Script ejecutado correctamente insert credo detalle "+contador);
				result = true;
			}
		} catch (SQLException e) {
			Logger.getLogger(CBEstadoCuentaDAO.class.getName()).log(Level.SEVERE, null, e);
	 ...
	 * 
	 * @param objModel
	 *            : objeto que trae todos los parametros a insertar en la tabla
	 */
 ...
 */
	...
	public boolean insertCuentaComercial(List<CBPagosModel> list) {
		...
		try {
			...
			Logger.getLogger(CBEstadoCuentaDAO.class.getName())
				.log(Level.INFO, "INSERT SISTEMA COMERCIAL = " + ConsultasSQ.INSERT_SISTEMACOMERCIAL_SQ);
			Logger.getLogger(CBEstadoCuentaDAO.class.getName())
				.log(Level.INFO, "Cantidad de registros antes del insert: " + list.size());
			...
			while (it.hasNext()) {
				...
				if ("0".equals(objModel.getFecEfectividad()) || "0".equals(objModel.getFecha())) {
					Logger.getLogger(CBEstadoCuentaDAO.class.getName())
						.log(Level.INFO, "datos ingresados para fecha " + objModel.getFecEfectividad() + "   "
								+ objModel.getFecha());
				}
	...
	/**
	 * Se encarga de cambiar una cadena con comas a tipo double sin comas
	 * 11,000.00-
	 * 
	 * @param cadena
	 *            : numero ingresado como tipo String
	 */
	...
	/**
	 * Las cadenas de texto con caracteres especiales se validan que sean
	 * numeros, se eliminan estos caracteres
	 * 
