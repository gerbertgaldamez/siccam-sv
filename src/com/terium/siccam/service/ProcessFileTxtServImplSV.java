 ...
 */
public class ProcessFileTxtServImplSV extends ControladorBase implements
		ProcessFileTxtService {

	private static Logger logger = Logger
			.getLogger(ProcessFileTxtServImplSV.class);
	/**
	 * 
	 */
	...
	public List<CBDataBancoModel> leerArchivo(BufferedReader bufferedReader,
			int idBanco, int idAgencia, int idConfronta, String user,
			String nombreArchivo, String tipo, BigDecimal comisionConfronta,
			int idAgeConfro) {
		...
		if (cvaidao.insertarArchivos(idMaestro, nombreArchivo, idBanco,
				idAgencia, user)) {
			...
			int contLinea = 0;
			int lineaEmpiezaLeer = objeDataBancoDAO
					.obtenerLineaLectura(idConfronta);
			String formatoFechaConfronta = objeDataBancoDAO
					.obtenerFormatoFechaConfronta(idConfronta); // CarlosGodinez
																// -> 28/08/2017
			/**
			 * 
			 * */
			try {
				...
				while ((strLine = bufferedReader.readLine()) != null) {
					if (contLinea >= lineaEmpiezaLeer) {
						...
						if (strLine1.trim().length() != 0) {
							...
							// logger.debug(methodName + "\nLinea: " +
							// strLine1);
							// logger.debug(methodName + " - Longitud: " +
							// strLine1.length());
							// logger.debug(methodName +
							// " - VALOR DE LA VARIABLE 'delimitador' = " +
							// delimitador);
							if (delimitador.compareTo("n/a") == 0) {
								...
								if (strLine1.length() > 8) {
									...
									String[] splitNomCl = this.nomenclatura
											.split(",");
									logger.debug(methodName
											+ " - variable longit = " + longit);
									logger.debug(methodName
											+ " - LONGITUD DE VARIABLE longit = "
											+ strLine1.length() + "\n");
									if (strLine1.length() == Integer
											.parseInt(longit)) {
										logger.debug(methodName
												+ " - Longitud: " + longit);
										...
										// CBDataBancoDAO();
										bancoModel.setcBCatalogoBancoId(Integer
												.toString(idBanco));
										bancoModel
												.setcBCatalogoAgenciaId(Integer
														.toString(idAgencia));
										bancoModel
												.setCBBancoAgenciaConfrontaId(Integer
														.toString(idAgeConfro));
										bancoModel.setCreadoPor(user);
										bancoModel.setTipo(tipo);
										bancoModel.setFechaCreacion(formato
										...
										bancoModel
												.setFormatofecha(getFormatoFecha());
										bancoModel
												.setComision(comisionConfronta);
										...
										for (int countRec = 0; countRec < splitNomCl.length; countRec++) {
											...
											String ambasPosReplace = ambasPosiciones
													.replace(" ", ",");
											String[] splitAmbasPosString = ambasPosReplace
													.split(",");
											...
											/*
											 * TA para telefonos ALFANUMERICOS
											 * 29/09/2021
											 */
											...
											if ("TA".equals(nomenCla)) {
												...
												if (telefonoAlfaNum.length() <= 10) {
													System.out
															.println("Telefono alfanumerico: "
																	+ telefonoAlfaNum);
													bancoModel
															.setTelefono(telefonoAlfaNum);
												}
											}
											// TE para telefonos especiales
											if ("TE".equals(nomenCla)) {
												String telefonoEspecial = strLine1
														.substring(
																Integer.parseInt(posicion1),
																Integer.parseInt(posicion2))
														.trim();
											...
											if ("M".equals(nomenCla)) {
												/**
												 * Editado ultima vez por Carlos
												 * Godinez - Qitcorp -
												 * 28/08/2017
												 */
												String strMonto = strLine1
														.substring(
																Integer.parseInt(posicion1),
																Integer.parseInt(posicion2));
												...
												logger.debug(methodName
														+ " - ** Monto antes de parsear: "
														+ strMonto);
												if (strMonto.substring(0, 1)
														.equals(".")
														|| strMonto.substring(
																0, 1).equals(
																",")) {
											...
											if ("R".equals(nomenCla)) {
												...
												logger.debug(methodName
														+ "Agencia: " + agencia);
												bancoModel
														.setCbAgenciaVirfisCodigo(agencia
																.trim());
											}

											if ("D".equals(nomenCla)) {
												String fecha = strLine1
														.substring(
																Integer.parseInt(posicion1),
																Integer.parseInt(posicion2));
												logger.debug(methodName
										...
										if (isDate(bancoModel.getFecha(),
												formatoFechaConfronta)
												&& ((bancoModel.getTelefono() != null && !bancoModel
														.getTelefono().equals(
																"")) || (bancoModel
														.getCodCliente() != null && !bancoModel
														.getCodCliente()
														.equals("")))) {
											dataBancoModels.add(bancoModel);
										} else {
											enviarDataSinProcesar(strLine1,
													nombreArchivo, user,
													idMaestro,
										...
										// + strLine1);
										enviarDataSinProcesar(strLine1,
												nombreArchivo, user, idMaestro,
												"Cantidad erronea de campos consulte la configuracion de la confronta");
									}

								} else if (Character.isDigit(aChar) == true) {
									logger.debug(methodName
											+ " - valida si es diguito: "
											+ aChar);
									String cambioFecha = strLine1.toString();
									CustomDate customDate = new CustomDate();
									setFormatoFecha(customDate.getFormatFecha(
											cambioFecha.trim(),
											this.formatoFecha));
								...
								if (strLine1.length() != 0) {
									int cantAgrup = 0;
									logger.debug(methodName
											+ " - Delimitador para lectura1: "
											+ this.delimitador1);
									logger.debug(methodName
											+ " - Numero de columnas configuradas: "
									...
									if (nomenclatura
											.contains(Constantes.COLUMN)) {
										logger.debug(methodName
												+ " - nomenclatura config columna");
										splitData = getData(strLine1,
												this.delimitador1, 5);
										logger.debug(methodName
												+ " Numero de columnas Archivo = "
												+ splitData.length);
										...
										isDataColumn = true;
											...
											.contains(Constantes.NOM_FECHA_TIPO5)) {
										logger.debug(methodName
												+ " - nomenclatura es fecha tipo5");
										splitData = getDataFechaTipo5(strLine1,
												this.delimitador1);
										logger.debug(methodName
												+ " - Se ha realizado depuracion de la cadena");
										cantAgrup = this.cantidadAgrupacion;
										splitDataValidos = new String[cantAgrup];

									} else {
										splitData = strLine1.split("\\"
												+ this.delimitador1);
									...
									// Examina que el numero de columnas de la
									// linea que se esta leyendo sea
									// mayor o igual a la cantidad de agrupacion
									// configurada
									if (splitData.length >= cantAgrup) {
										...
										if (splitDataValidos.length == cantAgrup) {
											logger.debug(methodName
													+ " - es valido la condicion splitDataValidos");
											// Si la linea es valida, se pasan
											// los
											// parametros a getBancoModel()
											getBancoModel(strLine1, idBanco,
													idAgencia, idConfronta,
													user, nombreArchivo,
													idMaestro, tipo,
													comisionConfronta,
													getFormatoFecha(),
											...
											logger.debug(methodName
													+ " - NO Cumple condicion: splitDataValidos ");
											enviarDataSinProcesar(strLine1,
													nombreArchivo, user,
													idMaestro,
				...
				Messagebox
						.show("Ha ocurrido un error al intentar procesar el archivo de confronta",
								"ATENCION", Messagebox.OK, Messagebox.ERROR);
				logger.error(methodName + " - " + "ha ocurrido un error: "
						+ e.getMessage());
			}
		} else {
	...
	private String[] getDataFechaTipo5(String cadena, String delimitador) {
		...
		logger.debug(methodName
				+ " - Inicia obtener Data para nomenclatura fecha tipo5");
			...
			for (String s : firstData) {
				if (StringUtils.isNumeric(s)) {
					...
					if (posicion == firstData.length - 3) {
						...
						logger.debug(methodName + " - Depuracion data fecha : "
								+ s);
					}
				}
				lastData[posicion] = s;
	...
	private String[] getData(String cadena, String delimitador, int posicion) {
		...
		if (fistData.length > 0) {
			...
			dataFinal = oderData(ArrayUtils.addAll(
					ArrayUtils.remove(fistData, fistData.length - 1), lastData));
		}
	...
	private String[] oderData(String[] allData) {
		...
		for (int i = 0; i < depurateData.length; i++) {
			logger.debug("getData() - Datos ORDENADOS ------>> "
					+ depurateData[i]);
		}
		return depurateData;
	}
	...
	private String getEndData(String data) {
		...
		logger.debug("getEndData() - Nombre : " + str1
				+ " - codigo colector : " + str2 + " -  Colector : " + str3);
	 ...
	 * @param isDataColumn
	 */
	private void getBancoModel(String strLine, int idBanco, int idAgencia,
			int idConfronta, String user, String nombreArchivo,
			String idMaestro, String tipo, BigDecimal comisionConfronta,
			String formatFecha, int idAgeConfro, String formatoFechaConfronta,
			int cantAgrup, String[] data, boolean isDataColumn) {
		...
		if (ArrayUtils.isNotEmpty(data) && isDataColumn) {
			splitData = data;
			splitNomCl = ArrayUtils.removeAll(this.nomenclatura.split(","), 0,
					1);
		...
		if (splitDataValidos.length == cantAgrup) {
			logger.debug(methodName
					+ " - validacion data validos y cantidad columnas valida");
			CBDataBancoModel bancoModel = new CBDataBancoModel();
			...
			bancoModel.setCBBancoAgenciaConfrontaId(Integer
					.toString(idAgeConfro));
			bancoModel.setCreadoPor(user);
			bancoModel.setTipo(tipo);
			...
			logger.debug(methodName + " - " + idBanco + " " + idAgencia + " "
					+ idConfronta + " " + user + " " + tipo + " "
			...
			for (int countRec = 0; countRec < splitDataValidos.length; countRec++) {
				...
				strData = strData.replaceAll("\"", ""); // Agrega Carlos Godinez
														// -> 27/06/2017
				logger.debug(methodName + " - validando nomenclatura : "
						+ valueToSave);
				// Telefono Tipo 1
				if ("T".equals(valueToSave)) {
					logger.debug(methodName
							+ "\n##Valor que lleva en la nomeclatura T = "
							+ strData.trim());
					...
					if (telstr.length() == 8) {
						...
						if (telstr != null && !telstr.equals("")
								&& !telstr.equals("0") && telstr.length() < 8) {
							logger.debug(methodName
				...
				if ("TA".equals(valueToSave)) {
					logger.debug(methodName + " -  Valindando nomenclatura "
							+ valueToSave);
				...
				if ("CT".equals(valueToSave)) {
					...
					try {
						...
						telstr = numeros.substring(numeros.length() - 10,
								numeros.length());
					...
					if (telstr.length() == 8) {
						...
						logger.debug(methodName + " - Codigo Cliente: "
								+ telstr);
						bancoModel.setCodCliente(telstr.trim());
					}
					...
					// Agrega Carlos Godinez - 12/09/2017
					if (bancoModel.getCodCliente() == null
				...
				if ("C".equals(valueToSave)) {
					logger.debug(methodName + " - "
							+ "\n## Valor que lleva en la nomeclatura C = "
							+ strData.trim());
					String numeros = strData.trim();
					String numReplace = numeros.replace(".", ",");
					Object[] split = numReplace.split(",");
				...
				if ("M".equals(valueToSave)) {
					...
					logger.debug(methodName + " - Monto antes de parsear = "
							+ strData);
					if (strData.substring(0, 1).equals(".")
							|| strData.substring(0, 1).equals(",")) {
						strData = "0".concat(strData);
						logger.debug(methodName + " - Primer elemento: "
								+ strData.substring(0, 1) + " Nueva cadena: "
								+ strData);
					}
				...
				if ("D".equals(valueToSave)) {
					// Se quita este parseo y se deja el que tiene el insert con
					// to_date
				...
				if ("H".equals(valueToSave)) {
					...
					// se concatena la fecha obtenida anteriormente para cuando
					// viene la hora
					// separada
			...
			if (isDate(bancoModel.getFecha(), formatoFechaConfronta)
					&& ((bancoModel.getTelefono() != null && !bancoModel
							.getTelefono().equals("")) || (bancoModel
							.getCodCliente() != null && !bancoModel
							.getCodCliente().equals("")))) {
				dataBancoModels.add(bancoModel);
				logger.debug(methodName + " - se agrega data a DAO bancoModel");
			} else {
				logger.debug(methodName
						+ " - telefono es vacio o null, codigo clientes es nulo o vacio");
				enviarDataSinProcesar(strLine, nombreArchivo, user, idMaestro,
						"Cantidad erronea de campos consulte la configuracion de la confronta");
			}
		} else {
			logger.debug(methodName
					+ " - NO es valida cantidad de columna config con las leidas");
			enviarDataSinProcesar(strLine, nombreArchivo, user, idMaestro,
					"Cantidad erronea de campos consulte la configuracion de la confronta");
		}
	}
	...
	private void getConfrontaConf(int idConfronta) {
		...
		List<CBConfiguracionConfrontaModel> dataList = cccb
				.obtieneListaConfConfronta(idConfronta);
		int sizeList = dataList.size();
		// System.out.println("tama;o lista: " + sizeList);
		for (int countRec = 0; sizeList > countRec; countRec++) {
			CBConfiguracionConfrontaModel confrontaModel = (CBConfiguracionConfrontaModel) dataList
					.get(countRec);
			this.delimitador1 = confrontaModel.getDelimitador1();
			this.delimitador2 = confrontaModel.getDelimitador2();
			logger.debug("getConfrontaConf()" + " - cantidad agrupacion: "
					+ confrontaModel.getCantidadAgrupacion());
	...
	public void enviarDataSinProcesar(String strLine, String nombreArchivo,
			String user, String idMaestro, String causa) {
		logger.debug("enviarDataSinProcesar()"
				+ " - ENTRA A VALIDACION DE REGISTROS SIN PROCESAR");
		logger.debug("enviarDataSinProcesar()"
				+ " - DATO QUE SE AGREGA A SIN PROCESAR = " + strLine);
	...
	public boolean isDate(String fecha, String formatoDeseado) {
		...
		Date fec = null;
		try {
			logger.debug("isDate()" + " - Formato fecha para Oracle = "
					+ formatoDeseado);
			...
			formatoDeseado = formatoDeseado.replace("MI", "mm");
			formatoDeseado = formatoDeseado.replace("am", "a"); // CarlosGodinez
																// -> 30/08/2017
			formatoDeseado = formatoDeseado.replace("am", "a"); // CarlosGodinez
																// -> 30/08/2017
			formatoDeseado = formatoDeseado.replace("AM", "a"); // CarlosGodinez
																// -> 30/08/2017
			formatoDeseado = formatoDeseado.replace("a.m.", "a"); // CarlosGodinez
																	// ->
																	// 30/08/2017

			logger.debug("isDate()" + " - Formato fecha para Java = "
					+ formatoDeseado);
			SimpleDateFormat format = new SimpleDateFormat(formatoDeseado,
					Locale.US);
	...
	public String guardarInfArchivo(List<CBDataBancoModel> dataArchivo,
			int idBanco, int idAgencia, int idConfronta) {
