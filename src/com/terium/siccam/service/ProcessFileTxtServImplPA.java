 ...
 */
public class ProcessFileTxtServImplPA extends ControladorBase implements ProcessFileTxtService {
	/**
	 * 
	 */
	...
	public List<CBDataBancoModel> leerArchivo(BufferedReader bufferedReader, int idBanco, int idAgencia,
		...
		if (cvaidao.insertarArchivos(idMaestro, nombreArchivo, idBanco, idAgencia, user)) {
			Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.INFO,
			...
			try {
										...
										for (int countRec = 0; countRec < splitNomCl.length; countRec++) {
											...
											if ("T".equals(nomenCla)) {
												...
												try {
													...
													Logger.getLogger(ProcessFileTxtServImplPA.class.getName())
											...
											if ("M".equals(nomenCla)) {
												...
												if (isBigDecimal(cadena)) {
													BigDecimal monto;
													if (cadena.indexOf(".") != -1) {
														Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(
											...
											if ("R".equals(nomenCla)) {
												String agencia = strLine1.substring(Integer.parseInt(posicion1),
														Integer.parseInt(posicion2));
												Logger.getLogger(ProcessFileTxtServImplPA.class.getName())
														.log(Level.INFO, "Agencia: " + agencia);
				...
				Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.SEVERE, null, ex);
			} catch (Exception e) {
				Messagebox.show("Ha ocurrido un error al intentar procesar el archivo de confronta", "ATENCION",
						Messagebox.OK, Messagebox.ERROR);
				Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.SEVERE, null, e);
			}
		} else {
	...
	private void getBancoModel(String strLine, int idBanco, int idAgencia, int idConfronta, String user,
		...
		Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.INFO,
				"-----------------------------------");
		Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.INFO, "Nomenclatura: " + nomenclatura);
		Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.INFO, "splitData = " + splitData[0] + ".");
		Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.INFO,
		...
		if (splitDataValidos.length == this.cantidadAgrupacion) {
			...
			for (int countRec = 0; countRec < splitDataValidos.length; countRec++) {
				...
				if ("CT".equals(valueToSave)) {
					...
					strtran = numeros.substring(8, numeros.length());
					telstr = numeros.substring(0, 8);
					...
					Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.INFO, "Telefono: " + telstr);
				...
				// monto
				if ("M".equals(valueToSave)) {//ovidio
					// Editado por Carlos Godinez - Qitcorp - 24/08/2017
					if (strData.trim() != null && ! strData.trim().equals("") ) {
	...
	private void getConfrontaConf(int idConfronta) {
		...
		for (int countRec = 0; sizeList > countRec; countRec++) {
			...
			Logger.getLogger(ProcessFileTxtServImplPA.class.getName()).log(Level.INFO,
					"cantidad agrupacion: " + confrontaModel.getCantidadAgrupacion());
	...
	public boolean isDate(String fecha, String formatoDeseado) {
		...
		try {
			...
			formatoDeseado = formatoDeseado.replace("pm", "a"); // CarlosGodinez -> 30/08/2017
