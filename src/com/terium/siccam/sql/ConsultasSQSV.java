...
public class ConsultasSQSV {
	...
	private static String LISTADO_AGENCIAS = "SELECT CBAGENCIASVIRFISID cbAgenciaVirfiscid, "
			+ " CBCATALOGOAGENCIAID cbCatalogoAgenciaid, " + " CODIGO codigo, " + " NOMBRE nombre, "
			+ " DECODE(TIPO, '1', 'PRESENCIAL', '0', 'VIRTUAL') tipo, " + " CREADO_POR creadoPor, "
			+ " FECHA_CREADO fechaCreado, " + " MODIFICADO_POR modificadoPor, " + " FECHA_MODIFICADO fechaModificado"
			+ " FROM CB_AGENCIA_VIR_FIS " + " WHERE CBCATALOGOAGENCIAID = ?";
	...
	//Query -> 7
	private static String CONSULTA_CONCILIACION_BANCO = "select to_char(dia,'dd/MM/yyyy'), " + 
			"       cbcatalogoagenciaid, " + 
			"       nombre, " + 
			"       max(nvl(estado_prepago,0)) estado_prepago, " + 
			"       sum(nvl(confronta_banco_prepago,0)) confronta_prepago, " + 
			"       sum(nvl(estado_prepago,0)-nvl(confronta_banco_prepago,0)) dif_prepago, " + 
			"       max(nvl(estado_pospago,0)) estado_postpago, " + 
			"       sum(nvl(confronta_banco_postpago,0)) confronta_postpago, " + 
			"       sum(nvl(estado_pospago,0)-nvl(confronta_banco_postpago,0)) dif_postpago, " + 
			"       max(nvl(pagosdeldia,0)) pagosdeldia, " + 
			"       max(nvl(pagosotrosdias,0)) pagosotrosdias, " + 
			"       max(nvl(pagosotrosmeses,0)) pagosotrosmeses, " + 
			"       max(nvl(ReversasOtrosDias,0)) reversasotrosdias, " + 
			"       max(nvl(ReversasOtrosMeses,0)) reversasotrosmeses, " + 
			"       sum(nvl(pagosdeldia,0)+nvl(pagosotrosdias,0)+nvl(pagosotrosmeses,0)) total_dia, " + 
			"       sum(nvl(pagosdeldia,0)+nvl(pagosotrosdias,0)+nvl(pagosotrosmeses,0)+nvl(ReversasOtrosDias,0)+nvl(ReversasOtrosMeses,0)) total_general, " + 
			"       sum(nvl(estado_pospago,0)) estado_cuenta, " + 
			"       sum(nvl(pagosdeldia,0)+nvl(pagosotrosdias,0)+nvl(pagosotrosmeses,0)+nvl(ReversasOtrosDias,0)+nvl(ReversasOtrosMeses,0)-nvl(estado_pospago,0)) diferencia_total, " + 
			"       max(nvl(porc_comisionpost,0)) porcentaje_postpago, " + 
			"       max(nvl(confronta_comision_postpago,0)) comision_confronta_postpago, " + 
			"       max(nvl(comision_postpago,0)) comision_postpago, " + 
			"       max(nvl(comision_postpago-confronta_comision_postpago,0)) diferencia_comision_postpago, " + 
			"       max(nvl(porc_comisionpre,0)) porcentaje_prepago, " + 
			"       max(nvl(confronta_comision_prepago,0)) comision_confronta_prepago, " + 
			"       max(nvl(comision_prepago,0)) comision_prepago, " + 
			"       max(nvl(comision_prepago-confronta_comision_prepago,0)) diferencia_comision_prepago, " + 
			"       sum(round(nvl(confronta_comision_postpago+confronta_comision_prepago,0),2)) comisiontotal, " + 
			"       sum(round(nvl(confronta_banco_postpago-confronta_comision_postpago,0),2))  recafinalpost, " + 
			"       sum(round(nvl(confronta_banco_prepago-confronta_comision_prepago,0),2)) recafinalpre, " + 
			"       sum(round(nvl(confronta_banco_postpago-confronta_comision_postpago+confronta_banco_prepago-confronta_comision_prepago,0),2))  totalfinal  " + 
			" 		from cb_conciliacion_banco_vw "+ 
			"		where 1 =1 ";
	
	//Query -> 8
	private static String OBTIENE_AGENCIA_SQ = "select distinct B.cbcatalogoagenciaid, b.NOMBRE " + 
	...
	*/
	...
	private static String INSERT_SOCIEDAD_SQ = "INSERT INTO CB_ESTADO_CUENTA_SOCIEDAD(CBESTADOCUENTASOCIEDADID, "
			...
			+ " to_date(?,'dd/MM/yyyy'),?,?,?,?,?,?,?,null,?,?,?,to_date(?,'yyyy/MM'),to_date(?,'dd/MM/yyyy'),?,?,?,?,?,SYSDATE)";
